...
Morlana — Telegram Mini App «ИИ-Таролог»: пользователь задаёт вопрос, сервер тянет карты Таро, LLM стримит толкование; сколько карт и сколько циклов — решает файл расклада (у «Одной карты» — одна карта и 6 циклов), затем синтез и архив в историю. Интерфейс на русском.
...
12. В генераторе `/interpret` порядок записи фиксирован: сначала commit в Postgres, потом одна запись документа расклада в Redis (`cycle_data`, номер цикла и новое состояние — одной командой, `complete_cycle`), чтобы сбой БД не оставил расклад в `READY` без сохранённого цикла. Любая ошибка оставляет `INTERPRETATION`, откуда `/interpret` повторяется. Если строка цикла уже закоммичена, а сбой случился позже, повтор завершает цикл из сохранённой строки без нового вызова LLM: вторая вставка нарушила бы `uq_reading_cycles_session_cycle`.
...
- Значения состояний — ASCII-строки (`WAITING`, `QUESTION_ASKED`, `CARDS_DRAWN`, `INTERPRETATION`, `READY`, `COMPLETED`): `ReadingState` в `services/reading.py`, тип `ReadingStateValue` в `frontend/src/api/client.ts` (сравнения — в `hooks/useReadingSession.ts` и `components/ReadingScreen.tsx`), константы в `backend/tests/test_reading.py`.
- Карта в API — объект `DrawnCard {deck_id, card_id, position, reversed, name, image}`: `schemas/tarot.py` и `interface DrawnCard` в `frontend/src/api/client.ts`. Имена и картинки карт знает только манифест колоды (`backend/decks/<id>.json`); на клиенте знаний о картах нет, `name` и `image` всегда приходят с сервера.
...
- Нужны поднятые `db` и `redis`; работающие `backend` и `frontend` команда не трогает. Прогон занимает 2-3 минуты вместе с установкой dev-зависимостей (276 тестов на 22.09.2026; один пропускается вне CI — картинки колоды лежат во `frontend/public`, которого нет в backend-контейнере). Устройство стенда и CI — `README.md`, разделы «Тесты» и «CI».
- Что чем закреплено: `test_auth.py` — initData, register, login, `/me`, rate limit; `test_reading.py` — автомат, владение, `/start`, история, сбои LLM; `test_infra.py` — загрузочные проверки, старт только на head, совпадение моделей со схемой, уникальность цикла; `test_astrology.py` — `/astrology/bonus`; `test_llm_client.py` — клиент провайдера из настроек (URL, ключ, `OpenAI-Project`, заголовок логирования, модель); `test_llm_prompts.py` — правила персоны в промптах толкования и синтеза, очистка ответа `clean_llm_output`; `test_safety.py` — фразы, которые `is_crisis_message` ловит и не ловит, `CRISIS_REPLY` вместо LLM в `/interpret` и `/synthesis`; `test_decks.py` — манифест колоды, имена карт 1..78 как раньше, `DrawnCard`, RNG; `test_spreads.py` — схема и загрузка раскладов; `test_catalog.py` — `/spreads` и `/decks`; `test_migration.py` — перенос данных ревизии `f4a5b6c7d8e9` туда и обратно (гоняет alembic по `morlana_test`).
- Каждое изменение поведения закрывай тестом. Готовые шаги — в `tests/helpers.py` (`sign_init_data`, `auth_headers`, `register`, `start_reading`, `run_cycle`, `synthesize`, `parse_sse`, `db_rows`, `reading_document`); фикстуры `client`, `llm` (`llm.fail = True` — сбой провайдера, `llm.calls` — сделанные вызовы), `rate_limits_on`, `two_card_spread` и `tiny_deck` (расклад и колода, которых нет в репозитории: доказательство, что и то и другое добавляется одним файлом). ruff проверяет только корректность (`E9`, `F`).
...
- Эндпоинты расклада (`/start`, `/ask`, `/draw`, `/interpret`, `/next`, `/synthesis`, `/state`, `/active`, `/history`) живут под `/api/v1/tarot/reading`, каталог (`/spreads`, `/decks`) — под `/api/v1/tarot`. Автомат: `ReadingState` и `VALID_TRANSITIONS` в `services/reading.py`; предел циклов — не константа модуля, а `max_cycles` из файла расклада, его же отдаёт `/state`. Действия расклада отвечают состоянием целиком (`ReadingStateResponse`): один запрос на действие. Продуктовые правила и известные пробелы флоу — `instructions.md`, разделы 5 и 7.
- Переходы в роутах делай только через `_apply_transition`: неверное состояние -> 409, никогда 500 (`/interpret` без вопроса или без карт отвечает 400 ещё до перехода). Расклад в полёте берётся через `_get_live_session`, его расклад и колода — через `_session_spread` и `_deck_or_default`: удалённый файл не делает расклад неотвечающим, он доживает на умолчании с warning в логе.
- Хранение: Redis держит незавершённое одним ключом `reading:{sid}` — JSON-документ `{state, cycle, spread_id, deck_id, question, cards, cycle_data}` с одним TTL 3600 с, который обновляется на каждой записи (поэтому отдельного «продления» нет). Ключ в старом формате пяти ключей (`reading:{sid}:state|cycle|...`) не читается: такой расклад отвечает 404 — расклады, начатые до выката этапа 2, теряются, это принято. Postgres хранит долговечное (`tarot_sessions`, `reading_cycles`). Через час без действий расклад недоступен UI, следующий `/start` его заархивирует или удалит. `RedisService` глотает ошибки: отказ Redis выглядит как 404 `Reading not found`, сбой подключения при старте — только warning.
- `/synthesis` не живой стрим: ответ LLM собирается целиком, коммитится (`status='archived'`), затем проигрывается чанками. Сбой или пустой ответ LLM -> 502 `Synthesis failed`, расклад не архивируется, остаётся в `COMPLETED`, повтор допустим. Клиент ключ `error` в SSE игнорирует: о сбое `/interpret` экран узнаёт, перечитав `/state`.
...
- Ошибка провайдера LLM (у MiMo пустой баланс — 402; у Yandex AI Studio 402 не бывает, ждите 401/403 при проблеме с ключом, ролью или платёжным аккаунтом и 429 при исчерпании квот): `/interpret` завершится SSE-сообщением с ключом `error`, расклад останется в `INTERPRETATION`; `/synthesis` ответит 502. Это не баг кода.
...
- Расклад — один файл `backend/prompts/spreads/<id>.json`, колода — папка картинок `frontend/public/decks/<id>/` плюс манифест `backend/decks/<id>.json`. Имя файла и есть id. Кода для нового расклада или новой колоды менять не нужно; так это и проверяют фикстуры `two_card_spread` и `tiny_deck`.
- Оба реестра (`services/spreads.py`, `services/decks.py`) читаются при импорте и валидируются pydantic с `extra='forbid'`: опечатка в поле роняет старт контейнера, а не выдаёт половину расклада. Умолчания — `DEFAULT_SPREAD_ID = 'one-card'`, `DEFAULT_DECK_ID = 'rider-waite'`; удалить файл умолчания нельзя.
- В раскладе `len(positions)` должно равняться `card_count`; `key` позиции попадает в `DrawnCard.position`, `label` и `prompt_addition` — в промпт. Работают `system_prompt`, `synthesis_prompt`, `aggregation_constraints`, `max_cycles`, `max_tokens`, `temperature`, `allow_reversed`, `requires_question`. Поле `tier` объявлено, но нигде не проверяется: расклад с `"tier": "premium"` сегодня доступен всем (гейт — этап 3).
- `id` карты в манифесте — имя файла картинки без расширения (`maj00`, `cups01`); `image` собирается как `<image_base>/<id><card_extension>`, если карта не задала свой. `/start` отвечает 400, если расклад просит больше карт, чем есть в колоде.
- Старые колонки `reading_cycles.card_id` (число 1..78) и `card_name` по-прежнему заполняются по ПЕРВОЙ карте цикла — только ради совместимости и отката миграции; правда лежит в `reading_cycles.cards`. Соответствие «число <-> id карты» — `LEGACY_CARD_ORDER` в `services/decks.py`, менять его нельзя: это факт о старых строках.
...
backend/services/          auth, password, reading (автомат), redis, llm (клиент провайдера, промпты), spreads и decks (реестры-данные), safety (кризисные сообщения), tarot (RNG на secrets), zodiac
backend/models/user.py     User, TarotSession, ReadingCycle;  backend/schemas/tarot.py — Pydantic-схемы (DrawnCard и остальные)
backend/prompts/           persona.md, spreads/*.json (расклады);  backend/decks/*.json — манифесты колод;  backend/alembic/versions/ — линейная цепочка, один head
backend/tests/             conftest.py (стенд), helpers.py, test_*.py;  .github/workflows/ci.yml — CI
frontend/src/              App.tsx, api/client.ts (типы API), hooks/useTelegram.ts, hooks/useReadingSession.ts (весь жизненный цикл расклада), components/, styles/
...
Мёртвый код, на который нельзя опираться: `SKIP_ONBOARDING`, `VITE_SKIP_ONBOARDING`, `Paywall.tsx` + `paywall.css`, `routes/payments.py` (не монтируй как есть: его модель запроса не совпадает с тем, что шлёт `Paywall.tsx`), `services/zodiac.py` (достижим только через `/astrology/bonus`). Полумёртвое, но нужное: `tarot_sessions.spread_name` (дубль `spread_id`, его отдаёт `/history`) и `reading_cycles.card_id`/`card_name` — см. задачу 7.
